...
5. Jurors with compound last names (e.g. "LOPEZ CRUZ") that return no results are flagged for manual review rather than silently marked as not found — CCIS may have them indexed under only part of their last name
6. On completion, optionally merges all PDFs and the Excel report into one combined file
...
| `No matches found` | No match returned by CCIS |
| `Error - check manually` | An unexpected exception occurred on this row |
| `Warning - Could not confirm if there were matches, please manually check` | Juror has a compound last name and returned no results — may be indexed differently in CCIS |
...
2026-05-05 10:32:55 - Venire - WARNING - Juror ID: 1045 - compound last name 'LOPEZ CRUZ' flagged for manual review
